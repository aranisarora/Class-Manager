...
| `# The front desk` | What is a conversation no business holds yet, and what is the job there? (One-brain merge, 23 Aug 2026 — the desk's own prefix died with the second brain; whether the mode is active is the tail's fact. Reworded 24 Aug 2026 with 0051: the `visitor` role is gone, a desk arrival is a `prospect` and desk mode keys on `tenant.kind`) | `context.ts` |
...
| Added | When | Why |
| --- | --- | --- |
| **`player_attendance` in `SCHEMA_DOC`'s views, the question→view routing list at the top of that section, and provenance columns on `unmarked_billable_session` / `app.rate_on` / `class_roster.rate_source`** | 24 Aug 2026 | Rung 2, each landing with deletions, and the whole change measured at net +1.3% of the prefix. The view is 0053's mechanism for F-EW — the anti-join the model wrote as an inner join in three graded runs, non-empty and wrong — and the provenance columns are F-BK applied to the fallback that let ₹1,600 vanish from month close (`pricing = 'no rate on file'`, `unbilled_amount` NULL, never 0). The routing list is one line per question because the recorded failure was not ignorance of a view's existence but not reaching for it at composition time — `roster-tonight` rebuilt the four-table join beside prose that says not to. **The deletions in the same change**: the FK graph kept only its eight name-breaking edges plus the rule that derives the rest; the SQL-helpers block stopped restating Derived-values arguments; four near-verbatim DOMAIN_FACTS/PLATFORM/desk restatements of the `remember`, `handoff`, `schedule` and `reply` declarations went (the declaration is the decode point — doctrine 4/7's own precedent), with the generalizations kept; the "views inherit the reader" fact went from three authors to one plus pointers. **Deliberately NOT taken: compressing the permission grid's write half.** The audit is right that only 8 of its 72 write cells carry information beyond `admin`/`-` — but `check:rls-doc` makes the grid READ pg_policies per-verb in both directions, so the grid is not a second author, and the write cells are what a plan's preview is built against. Trading a machine-checked wall for ~2.3k chars and parser surgery on the gate that caught the family-privacy hole fails its own test. |
| **Three clauses on `PLATFORM`'s media bullet** — a shared contact card is the exception, it is a name and a number rather than a file, and how somebody sends one | 21 Aug 2026 | Rung 3, and it is a **narrowing of a false prohibition** rather than an addition — which is the only reason it passes test 1 at all. The bullet said *"You cannot open what they send you… so **never invite one**"*, and after `lib/messaging/contact-card.ts` that sentence is untrue about the one attachment that arrives as data: WhatsApp's `contacts` message is a name and a phone number in structured fields, already text before anybody touches it, and it now reaches the model inside the message body. This is the graveyard's own *"you cannot tell whether a given person's window is open"* entry in a second instance, and it is the worst form of the trap for the same reason: **not an unmentioned capability but a denied one**, which no reading of the prompt for what is missing will ever surface, because the sentence is right there answering the question. The standing *never invite one* would have kept the capability unreachable from the model's side forever. **Almost all of the first draft was cut, by test 1, in public**: *"both reach you inside their message"*, *"several at once"*, *"the number arrives already in the form you have to write it in"*, *"nobody transcribes it and nobody can mistype it"*, and a clause warning that a card is not a whole family. Every one is derivable from watching a single card arrive — it renders as an English sentence carrying the name and the E.164 number — so none of them earns bytes. What survives is the correction, plus the one thing the model cannot derive and would otherwise have to invent the wording of: **that the affordance is the paperclip and it is called Contact** (iOS, Android and Web label the flow differently, and the emulator's own menu says *Contact*, so naming it keeps the prompt and the surface in agreement), and **that it is less work for the person than typing a name and a number out** — a fact about which road is cheaper for the human, which is exactly `PLATFORM`'s own test of whether something changes what is worth attempting. **Deliberately NOT included: the literal `[shared a contact: … ]` shape.** *No worked chat examples* is a trap in the list above with a real incident behind it — bracket-formatted button rows in prefix prose were typeset into live message bodies as text nobody could tap — and the line is self-describing English, so the model reads it without a template to copy. Nothing here says when to suggest it; the situation is the model's to judge, which is the difference between this and the staged-outreach procedure two tables down. |
| **`tapBlock`, in the variable tail** — what a committed tap did, on the turn that must now describe it (below the boundary; listed here because it is a prompt addition and because it retires runtime copy) | 21 Aug 2026 | Rung 6 inverted, which is the only way this table gets a new row: **the mechanism was built and the block is what the built thing needed to say.** A tap used to compose its own receipt from `buildSummary` and send it with no model in the loop — F-CD is one message that both announced a write and denied it, because half was composed before the transaction and half after. The tap now hands its result to an ordinary turn, and this block is how the result arrives. Three properties are the whole of it. It is **in the tail, not the prefix**: it is per-turn by definition and varies on every byte. It **states and does not instruct** — who tapped what, that it ran before this round with nothing inferred, and the plan's result in `committedResult`'s shape, which is the same object `act` returns; the only imperatives in it are *do not run it again* and *these are yours to deal with*, both facts about this turn rather than advice about phrasing. And it is **a stated fact rather than a fabricated tool call**: seeding the message list with an assistant `act` the model never made would have been cheaper to write and would have made the model's picture of its own turn false, which is the exact thing `spokeAsTrailingProse` exists to prevent. It arrives with a deletion, as rung 2 asks: two runtime-authored sends leave the tap path in the same change. |
...
| **Two parameter descriptions on `send_invite_draft`** (now `send_invite`) | 17 Aug 2026 | Rung 1, ~80 characters, and it makes a built feature reachable for the first time. The operation has always served §9.1 step 2 — it takes `person_id` and mints the same `wa.me` deep link — but the declaration named neither id and the description implied a coach, so the model told an owner to *"share an invite link"* in prose and then worked out, one message later, that *"there isn't an explicit operation for family invites in the tools besides send_invite_draft (coach)"*. That is a correct reading of what it had been shown. **A parameter with no description is a capability with no advertisement** — the trap below, in the mirror. **21 Aug 2026:** the operation is now `send_invite` and the bot sends rather than drafts, so the link is a repair path (`as_draft`) instead of the route. Both descriptions stayed and one was added — for the same reason, and it is worth stating as the rule this row generalises to: **a parameter that is the wrong default needs a description saying so**, or the model reaches for the friction the change removed. |
