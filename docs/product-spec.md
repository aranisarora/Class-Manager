...
2. **Mint once, replay verbatim.** A button's action is authored at compose time, validated, stored. The tap replays the stored payload. **No model inference decides what a tap runs**, because a misread there commits someone to being somewhere. This governs the *write* and nothing else: once the transaction has closed, what the person is told about it is composed by the model on an ordinary turn, from the rows the plan actually produced. The two were fused for most of this product's life and the cost is recorded as F-CD — a runtime-authored receipt announcing a write and denying it in the same sentence, because half of it was composed before the transaction it described. The write is model-free; the sentence never should have been.
...
| 2 | **Agent loop** | Primitives (§14.1), `transaction(steps[])` (§14.2.1), `agent_task` (§13.1), action minting incl. `operation` and `steps` kinds, write-diff preview, layered context (§4), memory store and hot set (§5) | A tap's payload executes with no model deciding it, and the model then composes the receipt from what the transaction actually wrote. An expired action refuses. A multi-row write shows its diff before commit. **A rolled-back transaction has messaged nobody.** A self-scheduled task fires, runs under its minter's RLS, and expires |
