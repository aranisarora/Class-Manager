...
/**
 * The Vertex service account. `GOOGLE_APPLICATION_CREDENTIALS_JSON` is a path
 * in this build (`./.secrets/vertex-sa.json`) but inline JSON is accepted too,
 * because that is how it arrives in a deployed environment.
 */
export function serviceAccount(): ServiceAccount {
  ...
  if (raw.startsWith('{')) {
    ...
    const path = isAbsolute(raw) ? raw : resolve(findRoot(), raw)
    if (!existsSync(path)) {
      throw new AppError({
        code: 'service_account_missing',
        message: `GOOGLE_APPLICATION_CREDENTIALS_JSON points at ${path}, which does not exist.`,
      })
    }
  ...
  try {
    ...
    throw new AppError({ code: 'service_account_invalid', message: 'Service account JSON did not parse.', cause })
  }
...
/** Where `.env.local` and `.secrets/` live. Used to resolve relative paths. */
