...
| `sim.ts` | `npm run sim` — the agent week. A window is a SITTING: presence per seat is a seeded coin (`whoChecks` — nobody is dealt a schedule), everybody who checked speaks concurrently, one child process each (`_seat-worker.ts`, because `captureSql` keeps module-level state and two turns in one process blend their evidence), and whoever a reply reached answers it minutes later until they put the phone down or the caps bind (`MAX_EXCHANGES`/`MAX_BEATS`). The clock walk and every drain — the between-beat ones included — are recorded as turns of their own. `--preset smoke` is one day and one window; `--preset e2e` opens on the 25th (`--start` aims any run at the calendar) and runs the founding → month-end tally → payment arc, with `extra.lifecycle` closing the record as first-timestamps. `--budget-min` / `--budget-inr` stop it cleanly at a beat or window boundary with a complete record. `gc --hours N` reaps the worlds this driver left behind, and only those. `--events <ref>` and `--chaos <rate>` give the week a physical world to happen in — see `_events.ts` below. |
...
| `replay-ledger.ts` | A recorded run's tool calls, replayed through the REAL action-ledger rendering (`recentActions`), with what each renders as. Built the day a staged-preview guard shipped gate-green and could never fire (F-EL): a dead render is a number here before it is a month of drives. Prints, judges nothing. |
...
| `guard-value.ts` | What each runtime guard has actually caught. Reads current runs' `record.json` (adapter, 28 Aug 2026) as well as the pre-20-Aug per-arm archives — it used to read only the archive while appearing current. |
| `ops-cookie.mjs` | How a script gets through the ops gate. |
| `_env.ts`, `_danger.ts`, `_capture.ts`, `_derive.ts`, `_drive-config.ts`, `_personas.ts`, `_record-from-probe.ts`, `_seat.ts`, `_persona-agent.ts`, `_seat-worker.ts`, `_world-file.ts`, `_arrivals.ts`, `_events.ts`, `_findings.ts` | Shared. Leading underscore means "not a command" — `_findings.ts` is the exception, and runs. `_capture.ts` owns the one record shape and appends one line per turn to `turns.jsonl`; `_derive.ts` rebuilds everything else in the run directory from that log, so two seats can write at once and nothing has to read 800KB back to add a turn. `_drive-config.ts` resolves how long, how many, who and how much — preset, then `--config` file, then flags — and refuses an unknown flag rather than running the default under its name. `_record-from-probe.ts` converts `probe-model`'s per-arm files into the record shape. `_seat.ts` is the seat itself — blindfold, phone, clock walk and turn — extracted from `live.ts` so the human seat and the agent week cannot drift apart. `_persona-agent.ts` is the person in the seat when nobody is sitting in it: it turns a persona's goals into the next thing they type, given only what their phone shows. `openSeatModel` holds ONE `claude` process per person for the whole week, fed over stdin — the thread between Tuesday and Wednesday is the point, and it stopped paying ~4s of process start on every message (10.3s median a move became 6.4s). Each turn still sends only what arrived since they last looked, so the week is reachable and never re-read. `_seat-worker.ts` is one persona in one OS process for the length of a week, spawned by `sim.ts` over node IPC — it refuses to run without an IPC channel. `_world-file.ts` is the whole world: one JSON object holding the PEOPLE and what happens to them, and a builder that makes a sender, a front desk and a contact for each PRESENT person — no academy, no classes, nobody enrolled in anything, because the business is talked into existence by the people in it. The rest of `people[]` is the withheld CAST: allocated numbers with no rows, seeded into every seat's contact book (`seatContacts` in `_seat.ts`) so an invite is a card-share, seated with their own written brief when the product reaches their phone, or walking in on `arrives: N`. `worlds/` holds them, and `blank` is the default. It replaced the old world-spec builder, which wrote a settled academy in SQL before anybody spoke and forced a second mechanism to exist beside it: because the harness wrote the rows, every brief had to be DERIVED from them or it would describe a business that was not there. Delete the fixtures and that goes too — a brief cannot contradict a database that does not exist yet. `_arrivals.ts` seats whoever the business gained since the last window — which is now EVERYBODY except the founders, since a run opens with nobody in a business. Identity is the PHONE (one human is one number across tenants; a namesake gets a suffixed key), a role comes off three `exists` subqueries, and the words are the withheld cast's own brief when the number is one the world wrote — a fixed roster meant the product wrote to phones nobody was reading, which in a record cannot be told from being ignored. `_events.ts` is what happens TO the business during the week: six verbs (`absent`, `present`, `washout`, `away`, `lag`, `note`), a seeded `chaos` block, and the ground truth of who was physically at each session — which nothing had before, so the register the product asked a coach for was answered out of the seat model's imagination, and §6.4 bills off attendance. It writes NOTHING to the database, deliberately: the product has to learn a fact from somebody who types it, and `truth.json` beside the record is what lets `npm run truth` show the two accounts side by side. |
