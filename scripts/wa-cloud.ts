...
async function main(): Promise<void> {
  ...
  const needsGraph = ['subscribe', 'templates', 'templates:preview', 'send-test'].includes(command)
  ...
  switch (command) {
    ...
    default:
      ...
      console.log(
        '  status | link | subscribe | templates | templates:preview | admin <phone> <name> | send-test <phone>',
      )
      process.exitCode = 1
  }
}
